...
norm <- function(vector){
  ...
  else if (class(vector) == "numeric" || class(vector) == "matrix"){
...
radiant <- function(event1, event2){
  ...
  r1.start <- equatorial.to.cartesian(event.start.equatorial(event1))
  r1.end   <- equatorial.to.cartesian(event.end.equatorial(event1))

  r2.start <- equatorial.to.cartesian(event.start.equatorial(event2))
  ...
  #Check if the antiradiant was found by checking to see if the result is closer
  #  to the start of the event (thus its source) or the end of the event (thus
  #  its termination), which would not make sense for these purposes.
