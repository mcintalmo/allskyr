event <- setClass(
  "event",
  slots = c(
    name = "character", #Name of the event ev_YYYYMMDD_HHMMSSX0_1
    ...
    th  = "numeric", #spherical conversion of x,y coordinates using platefile
    phi  = "numeric", #spherical conversion of x, y coordates using platefile
    lsp  = "numeric", #UNKNOWN
...
setMethod(
  ...
  definition = function(events) {
    return(sapply(events, function(events) {
      julian.days <-
        2440587.5 + julian(event.date(events),
      ...
      capture.output(
        equatorial <-
          hor2eq(
            event.alt(events),
            event.az(events),
            event.julian.date(events),
            lat = event.latlon(events)[1],
            lon = event.latlon(events)[2],
            altitude = event.latlon(events)[3]
          ),
        file = 'NUL'
      )
