read.event <- function(event.file, verbose){
  ...
  if(any(data$cx == 0) || any(data$cy == 0)){ ### THIS DATA IS IN THE MASK, SOMETHING IS WRONG
...
save.events <- function(events, save.file = "./save-files/events.sav", verbose = FALSE){
  return(save(events, list = "events", file = save.file, verbose = verbose))
}
...
update.showers <- function(shower.file = c("./showers/shower_calendar_2010.txt", 
  ...
  shower.data <- lapply(shower.file, read.shower, year)
  
  shower <- NULL
  for(data.frame in shower.data){
    shower <- rbind(showers, data.frame)
  }
  
  return(shower)
}

save.showers <- function(showers, file = ".save-files/shower.sav"){
...
update.all.radiants <- function(radiant.path = "./save-files/radiants/", verbose=TRUE){
  events <- load.events()
  ...
  radiants <- lapply(showers, function(shower){
    ...
    if(verbose) print(shower.name)
    shower.file <- paste(radiant.path, shower.name, ".txt", sep="")
    
    if(!file.exists(shower.file)){
      shower.events <- find.events(shower$start.date, shower$end.date, events)
      rad <- shower.radiant(shower.events)
      write.table(rad, file=shower.file)
    }
  })
}

update.radiant <- function(shower, radiant.path="./save-files/radiants",
                           verbose=TRUE){
  shower.name <- paste(shower$abbrev, substr(shower$start.date, 1, 4), sep="")
  if(verbose) print(shower.name)
  shower.file <- paste(radiant.path, shower.name, ".txt", sep="")
...
load.radiant <- function(shower, radiant.path="./save-files/radiants/", 
                         verbose=TRUE){
  radiant.file <- paste(radiant.path, shower$abbrev, 
                       substr(shower$start.date, 1, 4), ".txt", sep="")
  return(read.table(radiant.file))
}